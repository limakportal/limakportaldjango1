...
from ..businessrules.views import HasPermission, GetResponsibleAdminPersonDetails, IsManager, \
    GetManagerPersonsDetailNoneSerializer
        ...
        if serializer.is_valid():
            ...
            if len(getPersonIdentityData) > 0:
                if 'id' in json.loads(json.dumps(getPersonIdentityData)):
                    ...
                    personIdentityEditSerializer = PersonIdentitySerializer(personIdentityObj,
                                                                            data=getPersonIdentityData)
            ...
            if len(getPersonBusinessData) > 0:
                if 'id' in json.loads(json.dumps(getPersonBusinessData)):
                    ...
                    personBussinessEddSerializer = PersonBusinessSerializer(personBusinessObj,
                                                                            data=getPersonBusinessData)
            ...
            if len(getPersonEducationData) > 0:
                for educationData in getPersonEducationData:
                    if 'id' in json.loads(json.dumps(educationData)):
                        ...
                        personEducationEditSerializer = PersonEducationSerializer(personEducationObj,
                                                                                  data=educationData)
                        ...
                        if personEducationAddSerializer.is_valid():
                            ...
                            transaction.savepoint_rollback(transaction)
                            return Response(personEducationAddSerializer.error_messages,
                ...
                for familyData in getPersonFamiltData:
                    if 'id' in json.loads(json.dumps(familyData)):
                        ...
                        if personFamilyAddSerializer.is_valid():
                            ...
                            transaction.savepoint_rollback(transaction)
                            return Response(personFamilyAddSerializer.error_messages,
                                            status=status.HTTP_400_BAD_REQUEST)
